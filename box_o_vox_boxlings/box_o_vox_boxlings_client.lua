...
local function check_override_ui(players, player_id)
   ...
   if client_player then
      if client_player.kingdom == "box_o_vox_boxlings:server:population:data:kingdoms:boxling" then
         -- hot load rayyas children ui mod
         _radiant.res.apply_manifest("/box_o_vox_boxlings/ui/manifest.json")
      end
   end
end
...
radiant.events.listen(box_o_vox_boxlings, 'radiant:init', function()
   radiant.events.listen(radiant, 'radiant:client:server_ready', function()
         trace_player_service()
      end)
   end)

return box_o_vox_boxlings