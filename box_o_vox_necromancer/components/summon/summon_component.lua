...
function SummonComponent:start()
    radiant.events.trigger_async(self._entity, 'box_o_vox_necromancer:combat:summon_cast')
    if not radiant.entities.has_buff(self._entity, 'box_o_vox_necromancer:data:buffs:summon_timer') then
        radiant.entities.add_buff(self._entity, 'box_o_vox_necromancer:data:buffs:summon_timer')
    end
end